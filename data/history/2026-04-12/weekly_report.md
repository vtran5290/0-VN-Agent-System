# Weekly Macro/Policy/Decision Packet — as-of 2026-04-12
_Report age: 0 day(s); market snapshot date: 2026-04-12_

**Data confidence:** High | missing: — | not_due_yet: — | warnings: —
**Market level source:** VNINDEX | **DistDays proxy:** VN30
## Global Macro + Fed
- FACTS (levels) + source tag:
  - UST 2Y: 3.78 — FRED DGS2, **value_date**=2026-04-09 (fred_dgs_daily_observation; not labeled as broker session close)
  - UST 10Y: 4.29 — FRED DGS10, **value_date**=2026-04-09 (fred_dgs_daily_observation)
  - **DXY reconstructed (FRED H.10, ICE-style):** 100.1573 (value_date=2026-04-03)
  - **DXY third-party proxy (Stooq/Yahoo):** 98.7 (value_date=2026-04-10) — cross-check, not licensed ICE; source=Yahoo Finance DX-Y.NYB
  - **DXY ICE official (env/manual only):** None (value_date=None)
  - **Legacy `global.dxy` (WoW driver):** 100.1573 — reconstructed if available, else third-party
  - **USD broad (FRED DTWEXBGS):** 120.6565 (value_date=2026-04-03)
...
- WHAT CHANGED (WoW):
  - UST 2Y Δ: 0.020000000000000018
  - UST 10Y Δ: 0.020000000000000462
  - Primary DXY Δ (legacy `dxy`): -19.333699999999993
  - USD broad (DTWEXBGS) Δ: None
  - Payroll MoM Δ persons: None
- INTERPRETATION: TBD when data is filled.
  - _Note:_ `ust_2y` value_date (2026-04-09) is **before** report as-of (2026-04-12).
  - _Note:_ `ust_10y` value_date (2026-04-09) is **before** report as-of (2026-04-12).
  - _Note:_ `usd_broad_index_fred` value_date (2026-04-03) is **before** report as-of (2026-04-12).
  - _Note:_ `nonfarm_payroll_level_thousands` value_date (2026-03-01) is **before** report as-of (2026-04-12).
  - _Note:_ `nonfarm_payroll_change_persons` value_date (2026-03-01) is **before** report as-of (2026-04-12).
  - _Note:_ `dxy_reconstructed` value_date (2026-04-03) is **before** report as-of (2026-04-12).
  - _Note:_ `dxy_third_party` value_date (2026-04-10) is **before** report as-of (2026-04-12).

## Vietnam Policy + Liquidity
- FACTS (levels):
  - OMO net: 25000 (verification=manual_entry, source=manual, detail=Operator manual: OMO net 25k tỷ VND; inject 25k, withdraw 0; LS 4.5%/năm — Mua kỳ hạn: 14ng 5000ty + 35ng 10000ty + 56ng 10000ty)
  ...
  - **SBV reference USD/VND:** 25105
- WHAT CHANGED (WoW):
  - OMO net Δ: None
  - Interbank ON Δ: 1.5499999999999998
...
- MARKET (levels): vnindex_level=1750.0, vn30_level=1928.23, distribution_days_rolling_20=6 (proxy: VN30)
- **Distribution (LB=25, refined):** VN30=6, HNX=3, UPCOM=2 → Composite=Elevated (leader=VN30)
- Breadth: VN30 trend_ok(>MA20)=True | HNX close=251.91, trend_ok(>MA20)=True | UPCOM close=128.2, trend_ok(>MA20)=True
- WHAT CHANGED (WoW):
  - VNIndex Δ: 53.75999999999999, Dist days Δ: 2
...
- Market risk flag (dist days): {'distribution_days_rolling_20': 6, 'distribution_days': {'vn30': 6, 'hnx': 3, 'upcom': 2}, 'dist_risk_composite': 'Elevated', 'dist_proxy_symbol': 'VN30', 'risk_flag': 'Elevated', 'force_reduce_gross': False}
