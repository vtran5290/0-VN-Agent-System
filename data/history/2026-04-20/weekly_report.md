# Weekly Macro/Policy/Decision Packet — as-of 2026-04-20
_Report age: 0 day(s); market snapshot date: 2026-04-20_

**Data confidence:** High | missing: — | not_due_yet: — | warnings: —
**Market level source:** VNINDEX | **DistDays proxy:** VN30
## Global Macro + Fed
- FACTS (levels) + source tag:
  - UST 2Y: 3.78 — FRED DGS2, **value_date**=2026-04-16 (fred_dgs_daily_observation; not labeled as broker session close)
  - UST 10Y: 4.32 — FRED DGS10, **value_date**=2026-04-16 (fred_dgs_daily_observation)
  - **DXY reconstructed (FRED H.10, ICE-style):** 98.6918 (value_date=2026-04-10)
  - **DXY third-party proxy (Yahoo DX-Y.NYB):** 5.5299 (value_date=2026-04-20) — cross-check, not licensed ICE; source=Yahoo Finance DX-Y.NYB
  - **DXY ICE official (env/manual only):** None (value_date=None)
  - **Legacy `global.dxy` (WoW driver):** 98.6918 — reconstructed if available, else third-party
  - **USD broad (FRED DTWEXBGS):** 118.8552 (value_date=2026-04-10)
...
- WHAT CHANGED (WoW):
  ...
  - UST 10Y Δ: 0.05000000000000071
  - Primary DXY Δ (legacy `dxy`): -20.7992
  - USD broad (DTWEXBGS) Δ: None
  - Payroll MoM Δ persons: None
- INTERPRETATION: TBD when data is filled.
  - _Note:_ `ust_2y` value_date (2026-04-16) is **before** report as-of (2026-04-20).
  - _Note:_ `ust_10y` value_date (2026-04-16) is **before** report as-of (2026-04-20).
  - _Note:_ `usd_broad_index_fred` value_date (2026-04-10) is **before** report as-of (2026-04-20).
  - _Note:_ `nonfarm_payroll_level_thousands` value_date (2026-03-01) is **before** report as-of (2026-04-20).
  - _Note:_ `nonfarm_payroll_change_persons` value_date (2026-03-01) is **before** report as-of (2026-04-20).
  - _Note:_ `dxy_reconstructed` value_date (2026-04-10) is **before** report as-of (2026-04-20).

## Vietnam Policy + Liquidity
- FACTS (levels):
  - OMO net: 15000 (verification=parsed, source=sbv, detail=SBV nghiệp vụ thị trường mở (HTML scrape))
  ...
  - **SBV reference USD/VND:** 25103
- WHAT CHANGED (WoW):
  - OMO net Δ: None
  - Interbank ON Δ: -0.26000000000000023
...
- MARKET (levels): vnindex_level=1837.11, vn30_level=2009.04, distribution_days_rolling_20=5 (proxy: VN30)
- **Distribution (LB=25, refined):** VN30=5, HNX=5, UPCOM=4 → Composite=Elevated (leader=VN30)
- Breadth: VN30 trend_ok(>MA20)=True | HNX close=257.33, trend_ok(>MA20)=True | UPCOM close=129.85, trend_ok(>MA20)=True
- WHAT CHANGED (WoW):
  - VNIndex Δ: 140.8699999999999, Dist days Δ: 1
...
- Market risk flag (dist days): {'distribution_days_rolling_20': 5, 'distribution_days': {'vn30': 5, 'hnx': 5, 'upcom': 4}, 'dist_risk_composite': 'Elevated', 'dist_proxy_symbol': 'VN30', 'risk_flag': 'Elevated', 'force_reduce_gross': False}
