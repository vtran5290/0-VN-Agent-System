# Weekly Macro/Policy/Decision Packet — as-of 2026-04-25
_Report age: 0 day(s); market snapshot date: 2026-04-25_
...
- FACTS (levels) + source tag:
  - UST 2Y: 3.83 — FRED DGS2, **value_date**=2026-04-23 (fred_dgs_daily_observation; not labeled as broker session close)
  - UST 10Y: 4.34 — FRED DGS10, **value_date**=2026-04-23 (fred_dgs_daily_observation)
  - **DXY reconstructed (FRED H.10, ICE-style):** 97.9315 (value_date=2026-04-17)
  - **DXY third-party proxy (Yahoo DX-Y.NYB):** 77903.25 (value_date=2026-04-25) — cross-check, not licensed ICE; source=Yahoo Finance DX-Y.NYB
  - **DXY ICE official (env/manual only):** None (value_date=None)
  - **Legacy `global.dxy` (WoW driver):** 97.9315 — reconstructed if available, else third-party
  - **USD broad (FRED DTWEXBGS):** 118.0795 (value_date=2026-04-17)
...
- WHAT CHANGED (WoW):
  - UST 2Y Δ: 0.07000000000000028
  - UST 10Y Δ: 0.07000000000000028
  - Primary DXY Δ (legacy `dxy`): -21.5595
  - USD broad (DTWEXBGS) Δ: None
  - Payroll MoM Δ persons: None
- INTERPRETATION: TBD when data is filled.
  - _Note:_ `ust_2y` value_date (2026-04-23) is **before** report as-of (2026-04-25).
  - _Note:_ `ust_10y` value_date (2026-04-23) is **before** report as-of (2026-04-25).
  - _Note:_ `usd_broad_index_fred` value_date (2026-04-17) is **before** report as-of (2026-04-25).
  - _Note:_ `nonfarm_payroll_level_thousands` value_date (2026-03-01) is **before** report as-of (2026-04-25).
  - _Note:_ `nonfarm_payroll_change_persons` value_date (2026-03-01) is **before** report as-of (2026-04-25).
  - _Note:_ `dxy_reconstructed` value_date (2026-04-17) is **before** report as-of (2026-04-25).

## Vietnam Policy + Liquidity
- FACTS (levels):
  - OMO net: 10000 (verification=parsed, source=sbv, detail=SBV nghiệp vụ thị trường mở (HTML scrape))
...
- WHAT CHANGED (WoW):
  - OMO net Δ: None
  - Interbank ON Δ: 0.13999999999999968
...
- MARKET (levels): vnindex_level=1853.29, vn30_level=2011.42, distribution_days_rolling_20=4 (proxy: VN30)
- **Distribution (LB=25, refined):** VN30=4, HNX=2, UPCOM=3 → Composite=Elevated (leader=VN30)
- Breadth: VN30 trend_ok(>MA20)=True | HNX close=251.95, trend_ok(>MA20)=False | UPCOM close=127.61, trend_ok(>MA20)=False
- **Broad market weak:** HNX and UPCOM both below MA20.
- **Index holding but breadth weak → breakout failure risk ↑**
- WHAT CHANGED (WoW):
  - VNIndex Δ: 157.04999999999995, Dist days Δ: 0
...
- Market risk flag (dist days): {'distribution_days_rolling_20': 4, 'distribution_days': {'vn30': 4, 'hnx': 2, 'upcom': 3}, 'dist_risk_composite': 'Elevated', 'dist_proxy_symbol': 'VN30', 'risk_flag': 'Elevated', 'force_reduce_gross': False}
