# Weekly Macro/Policy/Decision Packet — as-of 2026-05-10
_Report age: 0 day(s); market snapshot date: 2026-05-10_
...
- FACTS (levels) + source tag:
  - UST 2Y: 3.92 — FRED DGS2, **value_date**=2026-05-07 (fred_dgs_daily_observation; not labeled as broker session close)
  - UST 10Y: 4.41 — FRED DGS10, **value_date**=2026-05-07 (fred_dgs_daily_observation)
  - **DXY reconstructed (FRED H.10, ICE-style):** 97.9481 (value_date=2026-05-01)
  - **DXY third-party proxy (Yahoo DX-Y.NYB):** None (value_date=2026-05-10) — cross-check, not licensed ICE; source=Yahoo Finance DX-Y.NYB
  - **DXY ICE official (env/manual only):** None (value_date=None)
  - **Legacy `global.dxy` (WoW driver):** 97.9481 — reconstructed if available, else third-party
  - **USD broad (FRED DTWEXBGS):** 118.3926 (value_date=2026-05-01)
...
- WHAT CHANGED (WoW):
  ...
  - UST 10Y Δ: 0.14000000000000057
  - Primary DXY Δ (legacy `dxy`): -21.542900000000003
  - USD broad (DTWEXBGS) Δ: None
  - Payroll MoM Δ persons: None
- INTERPRETATION: TBD when data is filled.
  - _Note:_ `ust_2y` value_date (2026-05-07) is **before** report as-of (2026-05-10).
  - _Note:_ `ust_10y` value_date (2026-05-07) is **before** report as-of (2026-05-10).
  - _Note:_ `usd_broad_index_fred` value_date (2026-05-01) is **before** report as-of (2026-05-10).
  - _Note:_ `nonfarm_payroll_level_thousands` value_date (2026-04-01) is **before** report as-of (2026-05-10).
  - _Note:_ `nonfarm_payroll_change_persons` value_date (2026-04-01) is **before** report as-of (2026-05-10).
  - _Note:_ `dxy_reconstructed` value_date (2026-05-01) is **before** report as-of (2026-05-10).

## Vietnam Policy + Liquidity
- FACTS (levels):
  - OMO net: 3000 (verification=parsed, source=sbv, detail=SBV nghiệp vụ thị trường mở (HTML scrape))
  - Interbank ON: 5.72
  - Credit growth YoY: 1.0
  - **SBV reference USD/VND:** 25112
- WHAT CHANGED (WoW):
  - OMO net Δ: -7000
  - Interbank ON Δ: 1.4799999999999995
...
- MARKET (levels): vnindex_level=1915.37, vn30_level=2074.06, distribution_days_rolling_20=3 (proxy: VN30)
- **Distribution (LB=25, refined):** VN30=1, HNX=4, UPCOM=5 → Composite=Elevated (leader=VN30)
- Breadth: VN30 trend_ok(>MA20)=True | HNX close=246.49, trend_ok(>MA20)=False | UPCOM close=127.44, trend_ok(>MA20)=False
- **Broad market weak:** HNX and UPCOM both below MA20.
- **Index holding but breadth weak → breakout failure risk ↑**
- WHAT CHANGED (WoW):
  - VNIndex Δ: 219.12999999999988, Dist days Δ: -1
...
- Market risk flag (dist days): {'distribution_days_rolling_20': 3, 'distribution_days': {'vn30': 1, 'hnx': 4, 'upcom': 5}, 'dist_risk_composite': 'Elevated', 'dist_proxy_symbol': 'VN30', 'risk_flag': 'Elevated', 'force_reduce_gross': False}
