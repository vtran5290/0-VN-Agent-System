...
def _mode_comparison(df: pd.DataFrame, near_up: float, near_dn: float) -> pd.DataFrame:
    """
    Compare 3 operational modes on the realistic validation data:
      A — symmetric ±7%
      B — asymmetric hard filter (proposed)
      C — labels only (no hard block except >+14%)
    """
    rows = []
    for mode, fn in [
        ("A_symmetric_7pct",   lambda r: abs(r["pct_drift"]) <= 0.07),
        ("B_asymmetric_hard",  lambda r: r["pct_drift"] >= -near_dn and r["pct_drift"] <= near_up),
        ("C_labels_no14pct",   lambda r: r["pct_drift"] <= 0.14),
    ]:
...
def _write_recommendation(results: list[dict]) -> None:
    ...
    for res in results:
        ...
        far_cliff = (not far_bucket.empty
                     and far_bucket["mean_net"].mean() < bsl_mean * 0.30)
        ...
        lines.append(
            ...
            f"{'YES — label as Stretched, do not hard-block' if stretch_viable else 'NO — hard-block at +{near_up:.0%}'}\n\n"
        )
        lines.append(
            f"3. **>+14% hard reject confirmed?** "
            f"{'YES — cliff confirmed' if far_cliff else 'PARTIAL — check data'}\n\n"
        )
        lines.append(
            f"4. **Recommendation**: "
            f"{'Mode C (labels + hard reject >14%) with downside widening for A3' if cname == 'A3_primary' else 'Mode B (hard filter) for S3 — tighter downside validated'}\n\n"
        )
    ...
    lines.append(
        ...
        "| Upside cap | +8% proposed | +8% proposed |\n"
        ...
        "| >+14% | Hard reject | Hard reject |\n"
        "| C_GK | Keep ±7% symmetric — no evidence | ← same |\n"
        "| Promote to paper-trade rule? | NO — scan triage only for now | NO |\n\n"
    )
    lines.append(
        "> **Final verdict**: Implement as Mode B scan filter with Mode C labeling.\n"
        "> Hard-block only entries outside the proposed window AND >+14%.\n"
        "> Label remaining entries by quality bucket for operator triage.\n"
