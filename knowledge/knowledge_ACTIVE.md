...
# v30 | 2026-07-08 | PA-009 CLOSED-NEGATIVE (council: Opus APPROVE + Fable HOLDS); exit-class gate HOLDS on first application; ChatGPT Trigger #5 verdict pending
# 6 CALIBRATED / 8 SOURCED / 4 AXIOMATIC / 1 INVALIDATED / 2 VN-SUBSUMED / 1 VN-THIN / 1 TESTED / 5 DEGRADING-REJECT / 1 VN-DEGENERATE
...
| PA-009 | 2R partial exit: exit half position when high[i] >= entry + 4×ATR14; run remaining half to original A3_RS exit | SOURCED | **CLOSED-NEGATIVE** (2026-07-08). Exit-class v2 gate (exact two-leg accounting): all 3 variants (1.5×/2.0×/2.5× 2R) PARKED — G1a FAIL, G1b FAIL, G1d_b FAIL (sub-B negative). OOS MAR collapsed 0.12–0.43 vs baseline 2.5233. MaxDD WORSENED 2–4× (−12% to −22% vs baseline −5.57%) → [ADVERSE-REVERSAL]. Trigger rate 74.8% → structural mechanic, not edge. v1 blended approx (1.88/5.57%) was screening proxy; exact two-leg invalidates it 4–15×. Council: Opus APPROVE(CLOSED-NEGATIVE) + Fable HOLDS. **Scope-of-invalidation: 2R partial-exit at 1.5×–2.5× R-multiple on A3_RS+S2@1.4x.** Different exit mechanic (time-based, vol-conditional) not foreclosed — requires fresh pre-reg. Pre-reg: knowledge/backtests/2026-07-08_pa009_exit_class_prereg_v2.md | 2026-07-08 |
