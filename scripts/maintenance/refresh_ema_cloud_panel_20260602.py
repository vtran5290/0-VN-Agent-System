"""One-shot: update ohlcv_panel_ext2012.parquet through 2026-06-02."""
...
def main() -> int:
    ...
    start_fetch = (panel_last + pd.Timedelta(days=1)).strftime("%Y-%m-%d")
    if start_fetch > END_FETCH:
        print(f"Panel already up to date ({panel_last.date()})")
        return 0
