...
UPCOM_TICKERS = frozenset({"BVB", "MSR"})
...
def _tv_scan(symbols: List[str]) -> Dict[str, Dict[str, Any]]:
    try:
        import requests
    except ImportError:
        logger.error("requests not installed — run: pip install requests")
        return {}
    payload = {"symbols": {"tickers": symbols}, "columns": _TV_QUOTE_COLUMNS}
    try:
        resp = requests.post(
    ...
    for item in data.get("data", []):
        ...
        ticker = sym.split(":")[-1] if ":" in sym else sym
        vals = item.get("d", [])
        row = dict(zip(_TV_QUOTE_COLUMNS, vals))
        row["symbol"] = sym
        out[ticker] = row
    return out
...
def fetch_quotes_tv(tickers: List[str]) -> Dict[str, Dict[str, Any]]:
    out = _tv_scan([_tv_symbol(t) for t in tickers])
    missing = [t for t in tickers if t not in out]
    for t in missing:
        tried = {_tv_exchange(t)}
        for exch in ("UPCOM", "HNX", "HOSE"):
            if exch in tried:
                continue
            retry = _tv_scan([f"{exch}:{t}"])
            if t in retry:
                out[t] = retry[t]
                logger.info("TV quote %s resolved via %s:%s", t, exch, t)
                break
    still_missing = [t for t in tickers if t not in out]
    if still_missing:
        logger.warning("TV quotes missing after exchange retry: %s", still_missing)
    return out
...
{"".join(at_rows)}
    ...
    scan_asof = str(daily_scan.get("as_of_date") or daily_scan.get("scan_date") or regime_date or "—")
