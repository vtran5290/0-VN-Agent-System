...
"""
Example:
    python -m regime.validate_regime --csv data/regime_log_2019_2020.csv
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate O'Neil-style market regime log.")
    parser.add_argument("--csv", required=True, help="Path to regime_log.csv")
    parser.add_argument(
        "--horizons",
        default="20,40",
...
def _summary_per_state(df: pd.DataFrame) -> None:
    print("\n=== SUMMARY PER MARKET STATUS ===")
    n_total = len(df)
    status_series = df["market_status"].astype(str)
    ...
    allow = df["allow_new_buys"].astype(bool)
    ...
    for state in sorted(status_series.unique()):
        ...
        if blocks_state:
            ...
            median_len = 0.0
            max_len = 0
        allow_true = bool(n_days) and bool(allow[mask].sum())
        pct_allow_true = (
            allow[mask].mean() * 100.0 if n_days and "allow_new_buys" in df.columns else 0.0
        )
        print(
...
def _compute_forward_returns(
    ...
    status = df["market_status"].astype(str)
...
def _print_forward_returns_summary(
    ...
    for h in horizons:
        print(f"\n-- Horizon H={h} days --")
        print(
            "state | med_ovl | med_nonovl | excess_ovl | excess_nonovl"
        )
        per_state = metrics[h]
        for state in sorted(per_state.keys()):
            ...
            print(
                f"{state:22s} | {mo:8.4f} | {mn:11.4f} | {eo:10.4f} | {en:13.4f}"
            )
...
def _detect_drawdown_events(
    ...
    states = df["market_status"].astype(str).values
...
def _drawdown_summary_and_lag(
    ...
    dd_confirmed = [e["dd_pct"] for e in events if e["market_status_at_peak"] == "confirmed_uptrend"]
    if dd_confirmed:
    ...
    print("\n=== DEFENSIVE TRANSITION LAG ===")
    status = df["market_status"].astype(str).values
    lags: List[int] = []
