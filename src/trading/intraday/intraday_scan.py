...
def _apply_intraday_policy(
    ...
    *,
    ...
    qmap = quotes_df.set_index("symbol") if not quotes_df.empty else pd.DataFrame()
    stale_syms = set()
    if not quotes_df.empty:
        stale_syms = set(quotes_df.loc[quotes_df["is_stale"] == True, "symbol"].astype(str))
...
def run_intraday_scan(
    ...
    *,
    ...
    if not capability.get("available"):
        ...
        empty = pd.DataFrame()
        if write_outputs:
            _write_outputs(empty, meta, cfg, mode, asof_timestamp, out_dir)
        return empty, meta
    ...
    scan_df = _apply_intraday_policy(
        scan_df, quotes_df, asof_timestamp=asof_timestamp, cfg=cfg, mode=mode, capability=capability,
    )
    scan_df["price_source_time"] = quotes_df.set_index("symbol")["timestamp"].reindex(scan_df["symbol"]).values
    ...
    if write_outputs:
        _write_outputs(scan_df, meta, cfg, mode, asof_timestamp, out_dir)
        write_intraday_report(scan_df, meta, quotes_df, cfg, mode, asof_timestamp, out_dir)
