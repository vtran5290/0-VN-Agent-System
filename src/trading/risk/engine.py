...
from src.trading.risk.sell_rules import _is_sell_exit, evaluate_sell_rules
...
class RiskEngine:
    ...
    def _evaluate_sell(
        ...
        reasons, rule_ids, decision = evaluate_sell_rules(
            proposal, ctx.portfolio, cfg, extra, ctx.pending_idempotency_keys
        )
        if decision == RiskDecision.PASS:
            return RiskVerdict(passed=True, reasons=[], rule_ids=[], decision=RiskDecision.PASS)
        return RiskVerdict(passed=False, reasons=reasons, rule_ids=rule_ids, decision=decision)
    ...
    def evaluate(
        ...
        if isinstance(cfg, LiveTradingConfig) and _is_sell_exit(proposal):
            return self._evaluate_sell(proposal, ctx, cfg, extra)
        if isinstance(cfg, LiveTradingConfig):
            return self._evaluate_buy(proposal, ctx, cfg, extra)
